    ...
    def _check_major_change(self, attrs):
        ...
        if attrs == ('brand_emblem', 'group'):
            ...
            def brand_groups(issue_or_revision):
                groups = []
                for emblem in issue_or_revision.brand_emblem.all():
                    groups.extend(emblem.group.all())
                return groups

            # As in the generic path: an add has no old value and a
            # delete has no new value.
            old_value = brand_groups(old) if old and not self.added else []
            new_value = brand_groups(new) if not self.deleted else []
            multi_valued = True
